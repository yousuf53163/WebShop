		...
		try {
			...
			if (sort.equals("quantity")) {
				rs = stmt.executeQuery(
						"SELECT leader, COUNT(leader), TRUNCATE(sum(current_price),2) FROM listing WHERE end_date<'"
								+ df.format(new java.util.Date())
								+ "' GROUP BY leader ORDER BY COUNT(*) DESC, sum(current_price) DESC LIMIT "
								+ limit);
			} else {
				rs = stmt.executeQuery(
						"SELECT leader, COUNT(leader), TRUNCATE(sum(current_price),2) FROM listing WHERE end_date<'"
								+ df.format(new java.util.Date())
								+ "' GROUP BY leader ORDER BY sum(current_price) DESC, COUNT(*) DESC LIMIT "
								+ limit);
			}
	%>
